...
def render_markdown(gpu_label, all_results):
    ...
    lines = [
        ...
        "",
        "**Baseline validity.** The \"vec\" compiled baseline (log-space cumsum / suffix-product) "
        "is checked against the exact sequential reference at every row, not assumed valid — it "
        "produces inf/nan from float32 underflow at these extreme lengths for some algorithms. "
        "Where it diverges, this is reported as `N/A (baseline invalid)`, not papered over with a "
        "misleading speedup ratio against broken output. Triton's correctness (via its chunked/flat "
        "auto-dispatch) is independently verified against the same exact reference at every row.",
        "",
