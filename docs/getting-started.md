...
By default (`boundary="ends_at"`) the accumulation resets the step *after* `dones[t] == 1` -- matching a raw Gymnasium `terminated | truncated` flag, where `dones[t]=1` means the episode ends at `t`, not that `t` starts a new one -- so each episode's cumulative sum is independent of the previous one. Pass `boundary="starts_at"` instead when the flag marks a new segment's *first* step, so the reset lands *at* the flagged step. An optional `seed_values` tensor of shape `(num_envs,)` sets the initial carry $C[-1]$ per environment (defaults to zero). Limited to `seq_len ≤ 131072`.
...
**`dones`** (used by `compute_eligibility_traces` and `compute_episodic_prefix_sum`): pass `terminated | truncated`. Neither of these forward-scan kernels distinguishes termination from truncation - both simply reset the trace/sum at the boundary - so there is no separate `truncateds` parameter for either. `compute_eligibility_traces` always resets the step *after* `dones[t] == 1` (the same convention as `terminateds` everywhere else in this package). `compute_episodic_prefix_sum` defaults to that same convention but also accepts `boundary="starts_at"` to reset *at* the flagged step instead, for flags that mark a new segment's first step - see [Episodic Prefix Sum](kernels/prefix_sum.md) for when to use which.
