...
def test_vtrace_performance():
    """
    Sweep over (num_envs, seq_len) configs comparing Triton vs torch.compile.
    ...
    header = (
        ...
        f"{'triton':>10} {'compiled':>10} {'vs compile':>12}"
    )
    print(header)
    print("-" * len(header))
    ...
    for num_envs, seq_len in BENCH_CONFIGS:
        ...
        triton_ms   = _bench_gpu(compute_vtrace_triton, *args, gamma=0.99)
        compiled_ms = _bench_cpu(compiled_vtrace, *args, gamma=0.99, n_warmup=n_warmup, n_iter=n_iter)
        speedup = compiled_ms / triton_ms
        all_speedups.append(speedup)

        print(f"{num_envs:>10} {seq_len:>8} {triton_ms:>9.3f}ms {compiled_ms:>9.3f}ms {speedup:>10.1f}x")
